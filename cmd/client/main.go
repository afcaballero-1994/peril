...
func main() {
	...
	for {
		...
		case "spam":
			if len(inuser) < 2 {
				fmt.Println("Need two arguments for spam command")
				continue
			}
			n, err := strconv.Atoi(inuser[1])
			if err != nil {
				fmt.Println("Incorrent argument", n)
				continue
			}
			for range n {
				msg := gamelogic.GetMaliciousLog()

				pubsub.PublishGob(publishCh, routing.ExchangePerilTopic,
